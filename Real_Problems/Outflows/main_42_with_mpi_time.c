 ...
 *********************************************************************** */
{
    ...
    print("  sizeof (char)     = %d\n", sizeof(char));
    print("  sizeof (uchar)    = %d\n", sizeof(unsigned char));
    print("  sizeof (short)    = %d\n", sizeof(short));
    print("  sizeof (ushort)   = %d\n", sizeof(unsigned short));
    print("  sizeof (int)      = %d\n", sizeof(int));
    print("  sizeof (*int)     = %d\n", sizeof(int_pnt));
...
/*
  print ("\n> Structure data type:\n");
  print ("  sizeof (CMD_LINE)   = %d\n", sizeof(Cmd_Line));
  print ("  sizeof (DATA)       = %d\n", sizeof(Data));
  print ("  sizeof (STATE_1D)   = %d\n", sizeof(State_1D));
  print ("  sizeof (GRID)       = %d\n", sizeof(Grid));
  print ("  sizeof (TIME_STEP)  = %d\n", sizeof(Time_Step));
  print ("  sizeof (OUTPUT)     = %d\n", sizeof(Output));
  print ("  sizeof (RUNTIME)    = %d\n", sizeof(Runtime));
  print ("  sizeof (RESTART)    = %d\n", sizeof(Restart));
  print ("  sizeof (RGB)        = %d\n", sizeof(RGB));
  print ("  sizeof (IMAGE)      = %d\n", sizeof(Image));
  print ("  sizeof (FLOAT_VECT) = %d\n", sizeof(Float_Vect));
  print ("  sizeof (INDEX)      = %d\n", sizeof(Index));
  print ("  sizeof (RBOX)       = %d\n", sizeof(RBox));
*/
...
/* -- initialize members of Time_Step structure -- */
    ...
    /* AYW -- set g_lastStep (actually not used - but kept as it might be useful) */
    g_lastStep = last_step;
    /* -- AYW */
...
/* --------------------------------------------------------
   ...
   ------------------------------------------------------- */
    ...
    //---DM 10Aug15: init_only only--//
    if (cmd_line.init_only == YES) {
        print("Done Initialisation. Quitting. \n");
        last_step = 1;
        /* AYW -- add global time version */
        g_lastStep = 1;
        /* -- AYW */
    }

    if (!cmd_line.init_only) print("> Starting computation... \n\n");
    //---DM---//
...
#endif
            /*
        print ("[dt/dt(adv) = %10.4e, dt/dt(par) = %10.4e, dt/dt(cool) = %10.4e]\n",
                     g_dt/dta, g_dt/dtp, g_dt/dtc);
                     */
            print("  dt(adv)  = cfl x %10.4e;\n", dta);
            print("  dt(par)  = cfl x %10.4e;\n", dtp);
            print("  dt(cool) =       %10.4e;\n", dtc);
        }
#endif
    ...
    /* ------------------------------------------------------
       ...
       if (!first_step && g_stepNumber%ini.log_freq == 0) {
         ...
         dtc = Dts.dt_cool;
          ...
          dtc = cg;
#endif
         print ("\t[dt/dta = %10.4e, dt/dtp = %10.4e, dt/dtc = %10.4e \n",
                  g_dt/dta, g_dt/dtp, g_dt/dtc);
       }
#endif
...
/* =====================================================================
   ...
   ===================================================================== */
    ...
    if (cmd_line.write && !cmd_line.init_only) { //---DM 10Aug15: Stop second write if only init_only---//
...
/* --------------------------------------------------------------
   ...
   -------------------------------------------------------------- */
    ...
    print ("! NextTimeStep(): dt is too small (%12.6e). Cannot continue.\n", dtnext);
        QUIT_PLUTO(1);
    }
 ...
 *********************************************************************** */
{
    ...
    /* AYW -- Include also global last step check */
    last_step = (fabs(t - ini->tstop) < 1.e-12 ? 1 : 0);
    last_step = last_step || g_lastStep;
    /* -- AYW */
 ...
 ********************************************************************** */
{
    ...
    if (check_dt || check_dn || g_lastStep) Analysis(d, grid);
